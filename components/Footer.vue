...
const { footer, socials, github } = useAppConfig()
...
<template>
  <div class="border-t border-gray-200 dark:border-gray-800 ">
    <footer class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 pb-20 md:pb-30 flex flex-col gap-12 md:gap-20 rounded-lg">
      <div class="grid md:grid-cols-2 gap-y-6 gap-x-12 md:gap-x-25">
        <div class="grow flex flex-col gap-6 max-w-sm">
          <NuxtLink to="/" class="flex items-center gap-2 text-xl">
            <UColorModeImage v-bind="{ ...footer?.logo }" class="w-7 h-7" />
      ...
      <div v-if="github.name" class="text-sm dark:text-gray-400 text-center">
        <span class="capitalize font-medium">{{ github.name }}</span> is part of the <NuxtLink to="https://unjs.io" class="hover:underline underline-offset-2">
          UnJS ecosystem.
        </NuxtLink>
      </div>
    </footer>
